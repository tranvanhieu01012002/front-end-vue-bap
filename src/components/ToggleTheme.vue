<template>
  <input
    :checked="checked"
    @input="event => inputCheck((event.target as HTMLInputElement).checked)"
    class="theme"
    type="checkbox"
    id="darkmode-toggle"
  />
  <label class="theme" for="darkmode-toggle">
    <svg
      version="1.1"
      class="sun"
...
const checked = ref(getTheme());

const inputCheck = (value: boolean) => {
  if (value) {
    setTheme("dark");
  } else {
    setTheme("light");
...
input.theme:checked + label.theme + .background {
  background: #242424;
}
...
input.theme:checked + label.theme svg.sun {
  fill: #7e7e7e;
}

input.theme:checked + label.theme svg.moon {
  fill: #fff;
}
</style>
